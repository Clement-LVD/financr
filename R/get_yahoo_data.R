...
if( n_symbs ==0) return(NULL)
  ...
  if(length(symbol) > 1){
    symbol <- symbol[[1]]
if( .verbose ) warning(immediate. = T, "Only one symbol should be passed to get_yahoo_data() and you have indicated several.
...
#1) temporal series within a list ####
...
col_to_retain <- c("open","close" #,"adjclose"
                   ,"low" , "high" , "volume" )
# add fake col
