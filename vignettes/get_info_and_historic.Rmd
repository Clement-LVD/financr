...
values_currencies <- last_currencies(keep = unknown_currencies, add_usd_values = T)

head(values_currencies)

```

See how to convert currencies and get last exchanges rates with the [Vignette of `get_changes()` and `last_currencies()`](https://clement-lvd.github.io/financr/articles/get_changes.html). 
