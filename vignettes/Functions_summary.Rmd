...
---
 `'financr'` only retrieves the data from Yahoo Finance and make no assumptions about the visualizations or analyses performed with these financial data. Consequently, `financr` dependencies are kept low.
...
fn <- data.frame(Minimal_example = c(
 ...
 ,"`search_assets(c('Saab', 'volvo'))`" ,"`search_asset_quick('Saab')`"
  ...
  , Input = c( "", "", "Free-text(s), e.g., companies names","Free-text(s)" ,"Free-text(s)"
...
, "Text(s) to verify"
 ...
 , "Validity of financial symbol(s)"
             ...
             , Data = c( rep("Latest**^‡^**", 2),"Latest**^†^**", "Latest*","Latest*", "Historic*",  "Latest*", "Historic*", "Latest*")
...
<!-- Other functions offered by these packages are sometimes more or less equivalent (e.g., `financr::valid_symbol()`) whereas other functions are specific to one of these packages. E.g., `'financr'` offer free-text search with `financr::search_summary()`, while `'quantmod'` offers financial data-analysis oriented functions and time series data visualization methods. Other packages are derivated from these 2 examples, e.g.,  which rely on quantmod and ans. -->
