...
This vignette is about the '`last_`' functions.
...
last_currencies(keep = "^Z", add_usd_values = T)
...
Some functions rely on web scraping (Yahoo Finance webpages), such as `financr::last_indices()` and `financr::last_crypto()` which respectively provide the latest financial values (in USD) for over 40 global market indices and 100 cryptocurrencies.
