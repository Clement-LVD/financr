...
- Get the historic of exchange rates, given currencies symbols. 
...
**Range and Interval Parameters.** Default is 1 row per day for a 1-year period if no `interval` and `range` are indicated. E.g., retrieve one observation per month over a five-year period.
...
- And/or filter symbols with the `keep` parameter, matching text with advanced perl regex (ignoring case).

```{r currencies2}
few_currencies <- last_currencies(keep = c("RON",  "Z..")
                                  , add_usd_values = T )
few_currencies
```
